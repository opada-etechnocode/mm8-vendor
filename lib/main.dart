...
Future<void> main() async {
      ...
      try{
        await Firebase.initializeApp(
          ...
          options: const FirebaseOptions(
            apiKey: "AIzaSyBS65XXwSv9Drcjxq2z2eqLXu3oj_sJ61I",
            projectId: "mm8market-237e9",
            messagingSenderId: "177056885354",
            appId: "1:177056885354:android:6a2fdd1fec086f5d0b7813"
          )
        );
      } catch(_) {
        await Firebase.initializeApp();
  ...
  try {
    ...
    if (remoteMessage != null) {
      final Map<String, dynamic> data = Map<String, dynamic>.from(remoteMessage.data);
      data['title'] ??= remoteMessage.notification?.title;
      data['body'] ??= remoteMessage.notification?.body;
      if ((data['order_id'] == null || data['order_id'].toString().isEmpty) && (remoteMessage.notification?.titleLocKey?.isNotEmpty ?? false)) {
        data['order_id'] = remoteMessage.notification!.titleLocKey;
        data['type'] ??= 'order';
      }
      body = NotificationBody.fromJson(data);
    }
  } catch(e) {
    if (kDebugMode) {
      print(e);
  ...
  try {
    ...
    if (body == null && (launchDetails?.didNotificationLaunchApp ?? false)) {
      final String? payload = launchDetails?.notificationResponse?.payload;
      if (payload != null && payload.isNotEmpty) {
        body = NotificationBody.fromJson(jsonDecode(payload));
      }
    }
  } catch (e) {
    if (kDebugMode) {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<LocalizationController>(
        builder: (context, localizationController, _) {
          ...
          AppFonts.update(localizationController.locale.languageCode);
        ...
        return MaterialApp(
          ...
          theme: Provider.of<ThemeController>(context).darkTheme ? dark(fontFamily: fontFamily)
              : light(
