...
class _MessageText extends StatelessWidget {
    ...
    return Flexible(child: Column(crossAxisAlignment: isMe ? CrossAxisAlignment.end : CrossAxisAlignment.start, children: [
      if(message.message != null && message.message!.isNotEmpty)
        InkWell(
          ...
          child: Container(
            margin: EdgeInsetsDirectional.only(start: isMe ? 56 : 8, end: isMe ? 8 : 56, top: 2, bottom: 2),
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            decoration: BoxDecoration(
              borderRadius: _getBorderRadius(),
              color: isMe
                  ? Theme.of(context).primaryColor
                  : Theme.of(context).hintColor.withValues(alpha: Provider.of<ThemeController>(context).darkTheme ? 0.18 : 0.10),
            ),
  ...
  BorderRadiusGeometry _getBorderRadius() {
    const Radius round = Radius.circular(18);
    const Radius tight = Radius.circular(6);

    if (isMe && (isSameUserWithNextMessage || isSameUserWithPreviousMessage)) {
      return BorderRadiusDirectional.only(
        topStart: round,
        bottomStart: round,
        topEnd: (isSameUserWithNextMessage && chatTime == "") ? tight : round,
        bottomEnd: (isSameUserWithPreviousMessage && previousMessageHasChatTime == "") ? tight : round,
      );
    } else if (!isMe && (isSameUserWithNextMessage || isSameUserWithPreviousMessage)) {
      return BorderRadiusDirectional.only(
        topEnd: round,
        bottomEnd: round,
        topStart: (isSameUserWithNextMessage && chatTime == "") ? tight : round,
        bottomStart: (isSameUserWithPreviousMessage && previousMessageHasChatTime == "") ? tight : round,
      );
    } else {
    ...
    return Column(
      ...
      children: [
        Directionality(
          ...
          child: Padding(
            padding: EdgeInsetsDirectional.only(start: (!isMe && isLTR) ? 40 : 0, end: (!isMe && !isLTR) ? 40 : 0),
...
class _MediaGridWidgetState extends State<_MediaGridWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsetsDirectional.only(bottom: Dimensions.paddingSizeExtraSmall, start: (!widget.isMe && isLtr) ? 40 : 0, end: (!widget.isMe && !isLtr) ? 40 : 0),
