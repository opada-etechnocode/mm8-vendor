...
class ChatScreen extends StatefulWidget {
  ...
  const ChatScreen({super.key, required this.userId, this.name = '', this.fromNotification = false});
    ...
    return PopScope(
      ...
      body: Consumer<ChatController>(builder: (context, chatController, child) {
        return Column(children: [
          CustomAppBarWidget(
            title: widget.name,
            onBackPressed: widget.fromNotification ? () {
              Navigator.of(context).pushAndRemoveUntil(
                MaterialPageRoute(builder: (_) => const DashboardScreen()),
                (route) => false,
              );
          ...
          SafeArea(top: false, child: SizedBox(
            height: chatController.openEmojiPicker ? 360 : 70,
